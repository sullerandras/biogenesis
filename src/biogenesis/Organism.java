...
public class Organism extends Rectangle {
	...
	/**
	 * Ivy parasitism effectivity
	 */
	protected double _ivyparasitism;
	/**
	 * Spin of spin and spring segments
	 */
	protected int _spin;
	/**
	 * Strength of drift is determined by the total amount of drift segments length
	 */
	protected int _drift;
	/**
	 ...
	 */
	protected int _homeX=-2;
	/**
	 * The Y coordinate used for drifting, either from the genetic code, or simply the birth place
	 */
	protected int _homeY=-2;
	/**
	 * The X coordinate used for drifting to base1X
	 */
	protected int _base1X=-2;
	/**
	 * The Y coordinate used for drifting to base1Y
	 */
	protected int _base1Y=-2;
	/**
	 * The X coordinate used for drifting to base2X
	 */
	protected int _base2X=-2;
	/**
	 * The Y coordinate used for drifting to base2Y
	 */
	protected int _base2Y=-2;
	/**
	...
	/**
	 * Indicates if this organism has protective segments
	 */
	protected boolean _isprotective;
	/**
	 * Indicates if this organism has movement segments
	 */
	protected int _canmove = 0;
	/**
	 * Indicates if this organism has teal segments
	 */
	protected boolean _canreact;
	/**
	 * Indicates if this organism can do autotrophy
	 */
	protected boolean _candoautotrophy;
	/**
	 ...
	 * Effective segment reactions, taken from the genetic code if alive or brown if dead.
	 */
	protected int[] _segcrimsonReaction;
	/**
	 * Effective segment reactions, taken from the genetic code if alive or brown if dead.
	 */
	...
	*/
	...
	final void segmentsCheckPlant() {
		...
		if (_ivyparasitism > 0) {
			if ((_isonlyivy) || (_isonlyc4 > 0)) {
				_ivyparasitism = (Math.log10(_ivyparasitism + 82.2))/2;
			} else {
				_ivyparasitism = (Math.log10(_ivyparasitism + 64.4))/2;
			}
		}
	 ...
	 */
	final void segmentsCreateEffects(boolean createall) {
		int i;
		_spin = 0;
		_drift = 0;
		_ivyparasitism = 0;
		...
		for (i=_segments-1; i>=0; i--) {
			...
			case ROSE:
				if (_transfersenergy == false) {
    			    ...
    			    _lengthfriend = (int)(Math.round(_geneticCode.getGene(i%_geneticCode.getNGenes()).getLength()*10)/10);
    			    _thetafriend = (int)(Math.round(_geneticCode.getGene(i%_geneticCode.getNGenes()).getTheta()*180.0/Math.PI)/10);
    			}
				if (_altruist) {
					_mphoto[i] = -4;
				} else {
					...
					if (_geneticCode.getModifiesblack() >= 5) {
						...
						if (_blackversion == 0) {
							...
							if (_blackversion <= 2) {
								...
								} else if (_blackversion == 24) {
									setColor(Color.MAGENTA);
								} else {
									setColor(Utils.ColorINDIGO);
								}
							}
						}
					}
							...
							for (j=_segments-1; j>=0; j--) {
								...
								case C4:
									if ((_canmove < 2) && ((_sporetime == 0) || (_geneticCode.getModifiesspore() >= 3))) {
									   _mphoto[j] = Utils.C4_ENERGY_CONSUMPTION * photomultiplier * 1.057 * (10.73 + _geneticCode.getGene(j%_geneticCode.getNGenes()).getLength());
									} else {
									   _mphoto[j] = Utils.C4_ENERGY_CONSUMPTION * photomultiplier * 1.019 * (10.73 + _geneticCode.getGene(j%_geneticCode.getNGenes()).getLength());
									}
									break;
								case MINT:
		...
		if (_ivyparasitism > 0) {
			if ((!_isaplant) && (_methanotrophy == 0) && (_isplankton == 0) && (_age == 0)) {
				_isonlyivy = true;
			}
			if ((_isonlyivy) || (_isonlyc4 > 0)) {
				_ivyparasitism = (Math.log10(_ivyparasitism + 82.2))/2;
		...
		if ((_transfersenergy) && (!_isaplant)) {
			if ((!_isaconsumer) && (!_isafungus)) {
				...
				if ((!_isinfectious) && (_earlyReproduceEnergy == 0) && (_blackversion >= -1) && (_age == 0)) {
					int c;
		...
		if (_yellowCounter > 0) {
			...
			if (_isonlyc4 == 2) {
				...
				_yellowCounter = (_yellowCounter / _symmetry) + (int) (fertility / 23);
				if (((_isinfectious) || (_iscoral)) && (!_isaconsumer) && (!_isaplant) && (!_isafungus)) {
	...
	public boolean inherit(Organism parent, boolean first) {
		...
		if (ok) {
            ...
            if (inheritReproduceEnergy <= 4) {
    			...
    			// Inherit drifting to the base of the parent
    			if (_drift > 0) {
    				if (inheritGeneticCode == parent._geneticCode) {
        				_switchdrift = parent._switchdrift;
        				if (_switchdrift == 1) {
        					if (_base1X < 0) {
        					    _switchdrift = 0;
        					}
    			    ...
    			    if (_sporeversion == 1) {
    					...
    					if ((parent._sporeversion == 3) && (inheritGeneticCode == parent._geneticCode)) {
    						if (_isaplant) {
    							...
    							if ((_isaconsumer) || (_isafungus) || (_isblond)) {
    								_isenhanced = false;
    								for (int i=0; i<_segments; i++) {
    									 _segColor[i] = Utils.ColorVISION;
    									 _mphoto[i] = 0;
    					...
    					if ((parent._sporeversion == 6) && (inheritGeneticCode == parent._geneticCode)) {
    						...
    						if ((_isaplant) && (_filterfeeding >= 0)) {
									...
									if (_methanotrophy > 0) {
										...
										if (_isonlyc4 > 0) {
											for (int i=0; i<_segments; i++) {
				                   			...
				                   			if (largeenough) {
				                   				_photosynthesis = ((15 * _mass) + (19.6 * (double)_symmetry))/Utils.GREEN_OBTAINED_ENERGY_DIVISOR;
											}
										} else {
											if (_blackversion <= -2) {
												_islime = false;
												...
												if ((_isakiller) || (_fallowversion > 0)) {
													...
													if (_islime) {
														for (int i=0; i<_segments; i++) {
							                   			...
							                   			if (largeenough) {
							                   			    _photosynthesis = ((14 * _mass) + (19.6 * (double)_symmetry))/Utils.GREEN_OBTAINED_ENERGY_DIVISOR;
							                   			    _leafphoto = ((19.5 * _mass) + (19.6 * (double)_symmetry))/Utils.GREEN_OBTAINED_ENERGY_DIVISOR;
														}
													} else {
														if (_forestphoto > 0) {
								                   			for (int i=0; i<_segments; i++) {
								                   			...
								                   			if (largeenough) {
								                   			    _photosynthesis = ((17 * _mass) + (19.6 * (double)_symmetry))/Utils.GREEN_OBTAINED_ENERGY_DIVISOR;
								                   			    _forestphoto = ((21 * _mass) + (19.6 * (double)_symmetry))/Utils.GREEN_OBTAINED_ENERGY_DIVISOR;
								                   			    _usepretoucheffects = true;
															}
														} else {
															if (_isenhanced) {
									                   			...
									                   			if (largeenough) {
									                   			    _photosynthesis = ((18 * _mass) + (19.6 * (double)_symmetry))/Utils.GREEN_OBTAINED_ENERGY_DIVISOR;
																}
															} else {
								...
								if ((_isaconsumer) || (_isafungus) || (_filterfeeding < 0)) {
									_isaplant = true;
		                   			for (int i=0; i<_segments; i++) {
									...
									if (largeenough) {
										_photosynthesis = (3.5 * _mass)/Utils.GREEN_OBTAINED_ENERGY_DIVISOR;
									}
								}
							}
							if ((_symmetry == 3) && (_geneticCode.getMirror() != 0)) {
			    ...
			    if (subcladecounter < Utils.CLADE_COMPLEXITY) {
			    	if (_geneticCode.getUpdateClade() < 0) {
			    		if (_geneticCode.getNGenes() > inheritGeneticCode.getNGenes()) {
				    		...
				    		_geneticCode._cladeID += "<" + Integer.toHexString(_world.getNewCladePart());
				    	} else if ((_geneticCode.getMirror() != inheritGeneticCode.getMirror()) && (_symmetry > 1)) {
			    		...
			    		if (_geneticCode.getUpdateClade() == 1) {
			    			...
			    			int i;
							for (i=_geneticCode._updateClade-1; i>=0; i--) {
								_geneticCode._cladeID += "|";
							}
							_geneticCode._cladeID += Integer.toHexString(_world.getNewCladePart());
			    		}
...
/*	private Vector2D translate(Point p, int step, int mirror) {
	 ...
	 * It tries to create one child as a spore and put it in the world.
	 */
	...
	public boolean move() {
		...
		if (dx!=0 || dy!=0 || dtheta!=0 || hasGrown!=0 || (_haseyes && _canmove == 2)) {
			...
			if (collision) {
				...
				if ((_drift > 0) && (active) && (Utils.random.nextInt(100)<8)) {
					...
					// Filter feeding with Plankton
					if (_filterfeeding > 0) {
						if ((!_haseyes) || (dx == dxbak)) {
							if (_spin > 0) {
								if (_world._detritus < 550) {
									if (Utils.random.nextInt(550) < _world._detritus) {
										_energy += _world.filterfeeding(((0.92 * (Math.abs(dx) + Math.abs(dy))) + (22.78278 * Math.abs(dtheta))) * _filterfeeding);
									}
								} else {
									_energy += _world.filterfeeding(((0.92 * (Math.abs(dx) + Math.abs(dy))) + (22.78278 * Math.abs(dtheta))) * _filterfeeding);
								}
				...
				if (_updateEffects > 0) {
					...
					if (_useextraeffects) {
						...
						if (_jadefactor > 0) {
							...
							if ((_leafphoto != 0) && ((_framesColor > 0) || (!_candoautotrophy))) {
								blockAutotrophyEffects();
							} else {
					    ...
					    if (_infectedGeneticCode == null) {
							...
							if (_energy > _reproduceEnergy && _growthRatio==1) {
								if ((_antiviral == 0) || (_lavender == 0)) {
									...
									if (useEnergy(Utils.MINT_ENERGY_CONSUMPTION/20)) {
										if (_lavender > 0) {
											lavenderinhibition();
										}
										if (_lavender <= 0) {
	 ...
	 */
	public void drift(boolean collided) {
		if (_switchdrift <= 0) {
			if ((_centerX > _homeX + 90) || (_centerX < _homeX - 90) || (_centerY > _homeY + 90) || (_centerY < _homeY - 90) || ((_isplankton > 0) && (_base1X < 0))) {
				...
				if ((Utils.random.nextBoolean()) || (collided)) {
					...
					if (_photosynthesis > 0) {
						if (_world._CO2 < 1125) {
							if (Utils.random.nextInt(1125) < _world._CO2) {
								_colonyPhotosynthesis += Utils.BOOST_ENERGY_CONSUMPTION * (Math.sqrt(_drift)*Utils.scale[_growthRatio-1]) * _photosynthesis;
							}
						} else {
							_colonyPhotosynthesis += Utils.BOOST_ENERGY_CONSUMPTION * (Math.sqrt(_drift)*Utils.scale[_growthRatio-1]) * _photosynthesis;
						}
						...
						if (_methanotrophy > 0) {
							if (_world._CH4 < 1125) {
								if (Utils.random.nextInt(1125) < _world._CH4) {
									_energy += _world.methanotrophy(0.125 * Utils.BOOST_ENERGY_CONSUMPTION * (Math.sqrt(_drift)*Utils.scale[_growthRatio-1]) * _methanotrophy);
								}
							} else {
								_energy += _world.methanotrophy(0.125 * Utils.BOOST_ENERGY_CONSUMPTION * (Math.sqrt(_drift)*Utils.scale[_growthRatio-1]) * _methanotrophy);
							}
	 ...
	 * enough oxygen in the atmosphere, false otherwise.
	 */
	public boolean useBreathing(double q) {
		if (_energy < q) {
			return false;
		}
	...
	/**
	 * Makes the organism spend an amount of energy
	 * producing carbon monoxide while using abilities.
	 *
	 * @param q  The quantity of energy to spend.
	 * @return  true if the organism has enough energy and there is
	 * enough oxygen in the atmosphere, false otherwise.
	...
	public void decay() {
		...
		if (alive) {
			if (_sporeversion == 1) {
				_age++;
				// Respiration process
				boolean canBreath = useBreathing(Math.min(0.003, _energy));
				if ((_age >> 8) >= _max_age || !canBreath) {
					// It's dead, but still may have energy
					die(null);
				} else {
				...
				_age++;
				// Respiration process
				boolean canBreath = useBreathing(Math.min(0.0026, _energy));
				if (((_symmetry == 1) && ((_age >> 8) >= _max_age)) || ((_symmetry > 1) && ((_age >> 8) > _max_age)) || !canBreath) {
					// It's dead, but still may have energy
					die(null);
				} else {
					if (_energy <= Utils.tol) {
				...
				if ((!_isinjured) || useDecayEnergy(Math.min(0.001, _energy))) {
					// Respiration process
					boolean canBreath = useBreathing(Math.min(0.001, _energy));
					if (!canBreath) {
						// It's dead, but still may have energy
						die(null);
					} else {
	...
	private final void mimic(int seg) {
		if (_blackversion <= 2) {
			...
			if (_blackversion == 3) {
				...
				_segColor[seg] = Utils.ColorCRIMSON;
			} else if (_blackversion == 8) {
	...
	/**
	 * Magenta segments are getting tricked into defending vs "Gray" by Black version 5 and 6
	 *
	 * @param blackOrganism  The organism that tricked
	 * all magenta segments of this organism
	 */
	public void tricked(Organism blackOrganism) {
		_healing = 0;
		for (int i=0; i<_segments; i++) {
			if (_segColor[i].equals(Color.MAGENTA)) {
                _segColor[i] = Utils.ColorDARKFIRE;
		...
		blackOrganism.setColor(Utils.ColorDARK);
	}
	/**
	...
	public void frozen(Organism freezingOrganism) {
		...
		if ((!_canreact) && (!isspin) && (_isonlyc4 != 2)) {
			_candodge =false;
			_dodge =false;
		}
		if (_ivyparasitism > 0) {
	...
	private final void touchMove(Organism org, Point2D.Double p, Line2D l, boolean thisOrganism) {
		...
		// Final speed
		dx = Utils.between(dx + j*nx/_mass, -5d, 5d);
	...
	/**
	 * Medium lavender shield is used
	 */
	private final void mediumlavendershield() {
		_lavender -= 1850;
		if (_lavender <= 0) {
			_lavender = 0;
		} else {
								...
								if (intersectsLine(bline) && line.intersectsLine(bline)) {
									...
									if ((((_parentID == org._ID || _ID == org._parentID) && !_generationbattle && !org._generationbattle) ||
										...
										if (_mphoto[i] == -1) {
											...
											touchCrimsonEffects(org,i,j);
											if (_candodge) {
												// Nothing to do with dodging, just stop piercing
												_candodge = false;
											} else {
												int k;
												ExLine2DDouble cline = new ExLine2DDouble();
												for (k = 0; k < org._segments; k++) {
													if ((k >= j) || (!org.active)) {
														break;
													} else {
														if (org._m[k]>=1) {
															cline.setLine(org.x1[k] + org._centerX, org.y1[k] + org._centerY,
																	org.x2[k] + org._centerX, org.y2[k] + org._centerY);
															if (intersectsLine(cline) && line.intersectsLine(cline)) {
																touchPiercingEffects(org,i,k);
															}
														}
													}
												}
										...
										} else if (org._mphoto[j] == -2) {
											org.touchCrimsonEffects(this,j,i);
											if (org._candodge) {
												// Nothing to do with dodging, just stop piercing
												org._candodge = false;
											} else {
												int l;
												ExLine2DDouble dline = new ExLine2DDouble();
												for (l = 0; l < _segments; l++) {
													if ((l >= i) || (!active)) {
														break;
													} else {
														if (_m[l]>=1) {
															dline.setLine(x1[l]+_centerX, y1[l]+_centerY, x2[l]+_centerX, y2[l]+_centerY);
															if (org.intersectsLine(dline) && bline.intersectsLine(dline)) {
																org.touchPiercingEffects(this,j,l);
															}
														}
													}
												}
	...
	private final void pretouchEffects(Organism org, int seg, int oseg) {
				...
				case BROWN:
					if ((_isenhanced) && (!_isgray) && (!org.alive)) {
						if ((org._age >> 8) > org._max_age) {
							_energy += _world.methanotrophy(1.5 * _methanotrophy);
						} else {
			...
			case DARK:
				if (_blackversion <= -2) {
					...
					case BROWN:
						break;
					default:
						if ((_isaplant) || (_isaconsumer) || (_isafungus) || (_isinfectious) || (_iscoral)) {
							if (_blackversion == -3) {
								for (int x = 0; x < _segments; x++) {
									switch (getTypeColor(_segColor[x])) {
									case DARK:
		...
		} else if ((_framesColor <= 0) && (_colonyPhotosynthesis > 0) && (!_isaplant) && (org._candoautotrophy) && (org.active)) {
	...
	/**
	 * Applies ivy parasitic photosynthesis and methanotrophy
	 *
	 * @param org  The organism which is touching.
	 * @param seg  Index of this organism's segment.
	 * @param oseg  Index of the other organism's segment.
	 */
	private final void ivyEffects(Organism org, int seg, int oseg) {
		switch (getTypeColor(_segColor[seg])) {
		case SPIKE:
		case DARKLILAC:
		case BROKEN:
			...
			if ((_mphoto[i] == -22) || (_mphoto[i] == -21) || ((_mphoto[seg] == -13) && (_mphoto[i] == -23))) {
				...
				default:
					if (org._mphoto[oseg] == -1) {
	                  switch (getTypeColor(org._segColor[oseg])) {
	                  case CRIMSON:
	                	    if (_segcrimsonReaction[seg] == 1) {
	                	    	stopmoving();
						    } else
	                    	if (_segcrimsonReaction[seg] == 2) {
	                    	if ((!_candodge && Utils.random.nextBoolean()) || (_candodge && Utils.random.nextInt(10)>2)) {
	    						dx=Utils.between((Utils.REACTION_VEL*_m[i]*(Math.ceil(_m[i])*(x1[seg]-x2[seg])+(x2[i]-x1[i])))/_mass, -5d, 5d);
	    						dy=Utils.between((Utils.REACTION_VEL*_m[i]*(Math.ceil(_m[i])*(y1[seg]-y2[seg])+(y2[i]-y1[i])))/_mass, -5d, 5d);
	    						dtheta=Utils.between(dtheta+Utils.randomSign()*_m[i]*Math.PI/_I, -Utils.MAX_ROT, Utils.MAX_ROT);
	...
	/**
	 * Applies the hostile effects produced by Crimson touching other segments.
	 * Non-piercing version
	 *
	 * @param org  The organism which is touching.
	 * @param seg  Index of this organism's segment.
	 * @param oseg  Index of the other organism's segment.
	 */
	private final void touchCrimsonEffects(Organism org, int seg, int oseg) {
		// Crimson segment: try to get energy from soft segments
		double takenEnergyCrimson = 0;
		...
		case GRAY:
			if (!_isenhanced) {
				if ((org._isaconsumer) || (org._isafungus) || (!org._isaplant)) {
					// Get energy depending on segment length
			...
			if ((org._dodge) && (org.useEnergy(Utils.DODGE_ENERGY_CONSUMPTION))) {
				...
				if (org._modifiesleaf) {
					if (org._framesColor > 0) {
						...
						// Get energy depending on segment length
						takenEnergyCrimson = Utils.between((0.01 * _m[seg]) / Utils.CRIMSON_ENERGY_CONSUMPTION, 0, org._energy);
		...
		case ROSE:
			...
			break;
		case DARK:
			if ((org._framesColor <= 7) && (org._blackversion >= -1)) {
		...
		// energy interchange
		if (takenEnergyCrimson > 0) {
			org._energy -= takenEnergyCrimson;
			_energy += takenEnergyCrimson;
	 ...
	 * @param oseg  Index of the other organism's segment.
	 */
	private final void touchPiercingEffects(Organism org, int seg, int oseg) {
		// Crimson segment: try to get energy from soft segments
		double takenEnergyCrimson = 0;
		if (org._hasdodged == false) {
		    org._hasdodged =true;
	...
	private final void touchEffects1(Organism org, int seg, int oseg) {
		...
		case SPIKEPOINT:
			...
			case OCHRE:
				...
				break;
			case CORAL:
				if ((org._isauburn) && (!org._isaplant) && (!org._isaconsumer) && (!org._isafungus)) {
					break;
				} else {
					if ((!_isaplant) && (!_isaconsumer) && (!_isafungus)) {
			    		if (useEnergy(Utils.SPIKE_ENERGY_CONSUMPTION)) {
				...
				if (_isenhanced) {
					...
					if (_isaplant) {
						if (useEnergy(Utils.SPIKE_ENERGY_CONSUMPTION)) {
							if (org._isenhanced) {
								// Get energy depending on segment length
								takenEnergySpike = Utils.between((3 * Math.sqrt(_m[seg])) * Utils.ORGANIC_OBTAINED_ENERGY, 0, org._energy);
							} else {
								// Get energy depending on segment length
								takenEnergySpike = Utils.between((5 * Math.sqrt(_m[seg])) * Utils.ORGANIC_OBTAINED_ENERGY, 0, org._energy);
							}
					...
					if ((_modifieslilac) || (org._isaconsumer) || (org._isafungus) || (org._plagueversion > 0) || (org._fallowversion > 0)
			    		...
			    		if (useEnergy(Utils.LILAC_ENERGY_CONSUMPTION)) {
			    			if (org._lavender > 0) {
								...
								takenEnergyLilac = Utils.between(((10 * Math.sqrt(_m[seg])) - (org._lavender/100)) * Utils.ORGANIC_OBTAINED_ENERGY, 0, org._energy);
		    	...
		    	if ((_modifieslilac) || (org._isaconsumer) || (org._isafungus) || (org._plagueversion > 0) || (org._fallowversion > 0)
		    		...
		    		if ((org._dodge) && (org._framesColor <= 0) && (org.useEnergy(Utils.DODGE_ENERGY_CONSUMPTION))) {
						...
						if (useEnergy(Utils.LILAC_ENERGY_CONSUMPTION)) {
							if (org._lavender > 0) {
								// Get energy depending on segment length reduced by the amount of lavender
								takenEnergyLilac = Utils.between(((5 * Math.sqrt(_m[seg])) - (org._lavender/100)) * Utils.ORGANIC_OBTAINED_ENERGY, 0, org._energy);
								if (takenEnergyLilac > 0) {
									// The other organism will be shown in dark lilac
									org.setColor(Utils.ColorDARKLILAC);
								} else {
		...
		case SILVER:
			...
			case GRASS:
				...
				break;
			case LEAF:
				if ((org._infectedGeneticCode != _geneticCode) && (org._nTotalInfected < _nTotalInfected) && ((!_isenhanced) || (!org._modifiesleaf))) {
					if ((org._dodge) && (org.useEnergy(Utils.DODGE_ENERGY_CONSUMPTION))) {
						org._hasdodged =true;
						org.setColor(Utils.ColorTEAL);
						setColor(Color.LIGHT_GRAY);
		...
		case DARKGRAY:
			...
			case DARKLILAC:
				if ((_mass > org._mass) && ((_growthRatio < org._growthRatio)
						|| ((!org._isaconsumer) && (!org._isafungus) && (!org._isaplant) && (_growthRatio <= org._growthRatio))) && (org._isaplant)) {
			...
			case SPIKE:
				if ((_mass > org._mass) && ((_growthRatio < org._growthRatio)
						|| ((!org._isaconsumer) && (!org._isafungus) && (!org._isaplant) && (_growthRatio <= org._growthRatio))) && ((org._isaplant) || (org._isenhanced))) {
					// Get the total energy of the smaller organism
					takenEnergyDarkgray = org._energy;
					org._energy -= takenEnergyDarkgray;
					_energy += takenEnergyDarkgray;
			...
			case BLUE:
				if ((_mass > org._mass) && ((_growthRatio < org._growthRatio)
						|| ((!org._isaconsumer) && (!org._isafungus) && (!org._isaplant) && (_growthRatio <= org._growthRatio)))) {
					// If the other segment is blue, it acts as a shield
					if (((!org._isaplant) || (org._framesColor <= 0)) && (org.useEnergy(Utils.BLUE_ENERGY_CONSUMPTION))) {
						setColor(Utils.ColorPLANKTON);
						org.setColor(Color.BLUE);
					} else {
						// Get the total energy of the smaller organism
			...
			case LEAF:
				if ((_mass > org._mass) && ((_growthRatio < org._growthRatio)
						|| ((!org._isaconsumer) && (!org._isafungus) && (!org._isaplant) && (_growthRatio <= org._growthRatio)))) {
					if ((org._dodge) && (org._canreact) && (org.useEnergy(Utils.DODGE_ENERGY_CONSUMPTION))) {
						org.setColor(Utils.ColorTEAL);
						setColor(Utils.ColorPLANKTON);
					} else {
						if ((!org._modifiesleaf) || (org._framesColor > 0)) {
			...
			case LAVENDER:
			    ...
			    break;
					...
					if ((_mass > org._mass) && ((_growthRatio < org._growthRatio)
							|| ((!org._isaconsumer) && (!org._isafungus) && (!org._isaplant) && (_growthRatio <= org._growthRatio)))) {
						if ((org._dodge) && (org._canreact) && (org._framesColor <= 0) && (org.useEnergy(Utils.DODGE_ENERGY_CONSUMPTION))) {
							org.setColor(Utils.ColorTEAL);
							setColor(Utils.ColorPLANKTON);
						} else {
							// Get the total energy of the smaller organism
		...
		case GRAY:
			...
			case CORAL:
				if ((org._isauburn) && (!org._isaplant) && (!org._isaconsumer) && (!org._isafungus)) {
					...
					if (((!_isaplant) || ((_isenhanced) && (_segmentpretoucheffects < 3))) && (!_isaconsumer) && (!_isafungus)) {
						if ((org._healing > 0) && (org._framesColor <= 8) && (org.useEnergy(Utils.MAGENTA_ENERGY_CONSUMPTION))) {
						    org.setColor(Color.MAGENTA);
						    setColor(Color.GRAY);
						} else {
							if ((org._healing > 0) && ((!org._isgray) || ((!org._isaconsumer) && (!org._isafungus)))) {
			...
			case BLUE:
				...
				break;
			case SKY:
				if ((org._skyversion == 2) || (org._isaplant)) {
					if (org.useEnergy(Utils.SKY_ENERGY_CONSUMPTION)) {
						org._segColor[oseg] = Utils.ColorDEEPSKY;
						org._mphoto[oseg] = -20;
						org._useextraeffects = true;
	...
	private final void touchEffects2(Organism org, int seg, int oseg) {
				...
				if (org._infectedGeneticCode != _geneticCode) {
					if ((org._isaplant) || (org._isaconsumer)) {
						if ((org._dodge) && (org.useEnergy(Utils.DODGE_ENERGY_CONSUMPTION))) {
							...
							if (_isaplant) {
								if ((_isenhanced) && (!_isakiller) && (org._antiviral == 0)) {
						    		if (useEnergy(Utils.WHITE_ENERGY_CONSUMPTION)) {
								        org.infectedBy(this);
								        org.setColor(Utils.ColorBLOND);
								        setColor(Color.WHITE);
			...
			case CORAL:
				if ((!_isaplant) && (!_isaconsumer) && (!_isafungus)) {
					if (useEnergy(Utils.VIOLET_ENERGY_CONSUMPTION)) {
						...
						org._mphoto[oseg] = -20;
						setColor(Utils.ColorVIOLET);
						if ((org._isonlyc4 == 3) || ((org._reproducelate > 0) && (!org._isinfectious) && (!org._isaplant) && (!org._isaconsumer) && (!org._isafungus))) {
							org._updateEffects = 2;
						}
						org._isinjured =true;
					}
		...
		case OLIVE:
			...
			case SPIKEPOINT:
				...
				break;
			case FALLOW:
				if ((!_isinfectious) || (!_isaplant) || (_isenhanced)) {
					if (useEnergy(Utils.OLIVE_ENERGY_CONSUMPTION)) {
						if (org._isaplant) {
						    org._segColor[oseg] = Utils.ColorBROKEN;
						} else {
		...
		case FALLOW:
			...
			case BARK:
				...
				if (org.active) {
					...
					if ((!org._isenhanced) || (_fallowversion == 1)) {
						if (org._lavender > 0) {
							if (_blackversion == -1) {
								org.weaklavendershield();
							} else {
			...
			case OCHRE:
				if (Utils.random.nextBoolean()) {
					if ((_isaplant && useEnergy(Utils.OCHRE_ENERGY_CONSUMPTION)) || (!_isaplant && useEnergy(Utils.OCHRE_ENERGY_CONSUMPTION/1000))) {
						...
						org.dx=Utils.between((org._centerX-_centerX)*0.2*_m[seg]*_m[seg]/org._mass, -5d, 5d);
						org.dy=Utils.between((org._centerY-_centerY)*0.2*_m[seg]*_m[seg]/org._mass, -5d, 5d);
						if (_isenhanced) {
							if (org._lavender > 0) {
								if (_blackversion == -1) {
									org.mediumlavendershield();
		...
		case CORAL:
					...
					if ((!org._transfersenergy) || (org._isinfectious) || (org._iscoral) || (org._isafungus)
						...
						if ((org._indigo > 0) && (!org._isafungus) && (!org._isblond) && (org._timeToReproduce <= org._timeToReproduceMax)) {
							...
							if ((org._dodge) && (org._energy >= 1) && (org.useEnergy(Utils.DODGE_ENERGY_CONSUMPTION))) {
								...
								if ((org._lavender <= 0) || (org._timeToReproduce > org._timeToReproduceMax)) {
			...
			case FRUIT:
				if (org._sporeversion == 1) {
					if ((org._framesColor > 0) || (_isaplant) || ((_hasgoodvision) && ((!org._iscoral) || (!org._hasgoodvision)))) {
						if (useEnergy(Utils.CORAL_ENERGY_CONSUMPTION)) {
							setColor(Utils.ColorCORAL);
							org.infectedBy(this);
							if (org._energy >= 1) {
					...
					if ((_hasgoodvision) && ((!org._iscoral) || (!org._hasgoodvision))) {
						if (useEnergy(Utils.CORAL_ENERGY_CONSUMPTION)) {
							setColor(Utils.ColorCORAL);
							org.infectedBy(this);
							if (org._energy >= 1) {
			...
			default:
				if (org.active) {
					...
					if ((org.alive) && ((_hasgoodvision) && ((!org._iscoral) || (!org._hasgoodvision)))) {
						if (useEnergy(Utils.CORAL_ENERGY_CONSUMPTION)) {
							setColor(Utils.ColorCORAL);
							org.infectedBy(this);
							if (org._energy >= 1) {
		...
		case SKY:
		    ...
		    break;
		case DARK:
			// Black segment: Versions 5 and 6 force an overreaction of organisms defense
			if (org._hasdodged == false) {
				org._hasdodged =true;
		    }
			switch (getTypeColor(org._segColor[oseg])) {
		    ...
		    case BROWN:
				break;
			case BLUE:
				if (!org._hasgoodvision) {
					if ((org.active) && (org.useEnergy((2 * Utils.BLUE_ENERGY_CONSUMPTION) + 0.1))) {
			...
			case SKY:
                if (!org._hasgoodvision) {
                	if (org.useEnergy((2 * Utils.SKY_ENERGY_CONSUMPTION) + 0.1)) {
    					org._segColor[oseg] = Utils.ColorDEEPSKY;
    					org._mphoto[oseg] = -20;
    					org._useextraeffects = true;
    					setColor(Utils.ColorDARK);
			...
			case OCHRE:
                if (!org._hasgoodvision) {
                	if ((org._isaplant && org.useEnergy(Utils.OCHRE_ENERGY_CONSUMPTION + 0.1)) || (!org._isaplant && org.useEnergy((Utils.OCHRE_ENERGY_CONSUMPTION/1000) + 0.1))) {
    					setColor(Utils.ColorDARK);
    					org.setColor(Utils.ColorOCHRE);
    				}
				}
				break;
			case OLIVE:
                if (!org._hasgoodvision) {
                	if (org.useEnergy((2 * Utils.OLIVE_ENERGY_CONSUMPTION) + 0.1)) {
    					org._segColor[oseg] = Utils.ColorDARKOLIVE;
    			    	org._mphoto[oseg] = -20;
    			    	org._useextraeffects = true;
    					setColor(Utils.ColorDARK);
			...
			case OLDBARK:
                if (!org._hasgoodvision) {
                	if ((org.active) && (org._mphoto[oseg] != -0.6)) {
    					for (int a = 0; a < org._segments; a++) {
    						switch (getTypeColor(org._segColor[a])) {
    						case OLDBARK:
    							org._mphoto[a] = -0.6;
    						...
    						case BARK:
    							setColor(Utils.ColorDARK);
    							org._segColor[a] = Utils.ColorOLDBARK;
    							org._photosynthesis -= org._mphoto[a]*Utils.scale[org._growthRatio-1];
    							org._mphoto[a] = -0.6;
    							break;
    						}
    					}
			...
			case SPIKEPOINT:
                if (!org._hasgoodvision) {
                	if (org._isenhanced) {
    			    	...
    			    	if ((org._antiviral > 0) && (org.useEnergy((2 * Utils.MINT_ENERGY_CONSUMPTION) + 0.1))) {
    			    		setColor(Utils.ColorDARK);
    			    		org.setColor(Utils.ColorMINT);
    			    	}
    				}
		...
		case MINT:
			...
			case BROWN:
				...
				break;
			case BLUE:
				if (((org._isinfectious) || (org._iscoral) || (org._plagueversion > 0) || (org._fallowversion > 0)) && (org.active)) {
	 ...
	 */
	private final void blockAutotrophyEffects() {
		    // Get chemotrophic energy from methane
		    if (_methanotrophy > 0) {
			    if ((_age & 0x07) == 0x00) {
			    	if (_candoautotrophy) {
		    ...
		    if (_photosynthesis > 0) {
		    	//Get sun's energy
			    if ((_leafphoto > 0) && (_framesColor == 1) && (!_color.equals(Utils.ColorDARKGREEN))) {
			    	...
			    	case SPIKE:
			    		if (!_isenhanced) {
			    			setColorfourFrames(Utils.ColorDARKGREEN);
			    		}
			    		break;
			    	default:
			    		setColorfourFrames(Utils.ColorDARKGREEN);
	...
	private final void plantsUpdateEffects() {
					...
					if (_mphoto[i] > -1) {
						...
						if (_mphoto[i] == -1) {
							...
							if (_mphoto[i] <= -21) {
								// Movement
								if (_mphoto[i] == -21) {
									if (_segColor[i].equals(Utils.ColorTEAL)) {
										addmaintenance -= 0.95 * _m[i];
									} else {
										// is modified drift
			...
			if (_photosynthesis > 0) {
				...
				if (_leafphoto != 0) {
					...
					if ((_framesColor > 0) || (!_candoautotrophy)) {
						if ((_leafphoto > 0) && (_framesColor == 1) && (!_color.equals(Utils.ColorDARKGREEN))) {
					    	switch (getTypeColor(_color)) {
					    	case RED:
					    	case FIRE:
	...
	private static final int JADE=8;
	private static final int GRASS=9;
	private static final int BARK=10;
	private static final int PURPLE=11;
	private static final int PLANKTON=12;
	private static final int RED=13;
	private static final int FIRE=14;
	private static final int ORANGE=15;
	private static final int MAROON=16;
	private static final int CRIMSON=17;
	private static final int PINK=18;
	private static final int CREAM=19;
	private static final int SILVER=20;
	private static final int SPIKE=21;
	private static final int LILAC=22;
	private static final int GRAY=23;
	private static final int VIOLET=24;
	private static final int OLIVE=25;
	private static final int SKY=26;
	private static final int BLUE=27;
	private static final int OCHRE=28;
	private static final int FALLOW=29;
	private static final int SPORE=30;
	private static final int WHITE=31;
	private static final int PLAGUE=32;
	private static final int CORAL=33;
	private static final int MINT=34;
	private static final int LAVENDER=35;
	private static final int MAGENTA=36;
	private static final int ROSE=37;
	private static final int CYAN=38;
	private static final int TEAL=39;
	private static final int DRIFT=40;
	private static final int SPIN=41;
	private static final int YELLOW=42;
	private static final int AUBURN=43;
	private static final int INDIGO=44;
	private static final int BLOND=45;
	private static final int FLOWER=46;
	private static final int DARKGRAY=47;
	private static final int GOLD=48;
	private static final int DARK=49;
	private static final int EYE=50;
	private static final int WINTER=51;
	private static final int OLDBARK=52;
	private static final int DARKJADE=53;
	private static final int DARKGREEN=54;
	private static final int DARKFIRE=55;
	private static final int DARKLILAC=56;
	private static final int DEEPSKY=57;
	private static final int DARKOLIVE=58;
	private static final int SPIKEPOINT=59;
	private static final int FRUIT=60;
	private static final int VISION=61;
	private static final int ICE=62;
	private static final int LIGHT_BLUE=63;
	private static final int LIGHTBROWN=64;
	private static final int GREENBROWN=65;
	private static final int BROKEN=66;
	private static final int DEADBARK=67;
	private static final int BROWN=68;
	private static final int getTypeColor(Color c) {
		if (c.equals(Color.GREEN))
			return GREEN;
		if (c.equals(Utils.ColorFOREST))
			return FOREST;
		if (c.equals(Utils.ColorIVY))
			return IVY;
		...
		if (c.equals(Utils.ColorMAROON))
			return MAROON;
		if (c.equals(Utils.ColorCRIMSON))
			return CRIMSON;
	...
	private final void setColorfourFrames(Color c) {
		_color = c;
		_framesColor = 4;
	}
