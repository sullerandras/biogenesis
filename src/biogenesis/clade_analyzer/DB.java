...
public class DB {
  ...
  public void insertCladeSummary(File summaryFile, String cladeId, int time, String geneticCode, int population) throws SQLException {
    executeUpdate("INSERT INTO " + CLADE_SUMMARIES_TABLE + " (SUMMARY_FILE_ID, CLADEID, TIME, GENETIC_CODE, POPULATION) " +
        "VALUES ("+
        "(SELECT ID FROM " + SUMMARY_FILES_TABLE + " WHERE FILENAME = '" +
        relativePath(summaryFile) + "'), '" + cladeId + "', " + time + ", '" + geneticCode + "', " + population + ")");
    executeUpdate("INSERT OR IGNORE INTO " + CLADES_TABLE + " (CLADEID, FIRST_SEEN_TIME, LAST_SEEN_TIME, GENETIC_CODE, MAX_POPULATION) " +
